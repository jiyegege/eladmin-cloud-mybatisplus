...
import com.roger.system.core.domain.*;
...
import com.roger.system.core.mapper.*;
...
public class RoleServiceImpl extends BaseServiceImpl<Role> implements RoleService {
    ...
    public List<GrantedAuthority> mapToGrantedAuthorities(Boolean isAdmin, Long userId) {
        ...
        Set<Role> roles = roleMapper.selectLink(userId);
        ...
        if (CollectionUtils.isNotEmpty(roleIds)) {
            permissions.addAll(menuMapper.selectByRoleIds(roleIds).stream()
                    .map(Menu::getPermission).filter(StringUtils::isNotBlank)
                    .collect(Collectors.toSet()));
        }
        return permissions.stream().map(SimpleGrantedAuthority::new).collect(Collectors.toList());
    }
